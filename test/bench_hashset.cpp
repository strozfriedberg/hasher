...
TEST_CASE("xxxxx") {
  const size_t HashLength = 20;
  MmapHolder h{mmap_file(NSRL)};
//  const size_t HashLength = 16;
//  MmapHolder h{mmap_file(VS)};
...
//  std::map<ssize_t, size_t> hist;

  std::array<std::pair<ssize_t, ssize_t>, 32> block_bounds;
  std::fill(
    block_bounds.begin(),
    block_bounds.end(),
    std::make_pair(
...
//    std::cout << i << ' ' << delta << '\n';
    ...
    const size_t bi = static_cast<size_t>(((double) i/hsinfo->hashset_size)*block_bounds.size());
...
//  MmapHolder h{mmap_file(VS)};
  ...
  const auto block_bounds = make_buckets<HashLength, 4>(h);
