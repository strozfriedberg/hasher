...
SCOPE_TEST(iterateHashset) {
  ...
  HashsetIterator i(txt, txt + std::strlen(txt));
...
SCOPE_TEST(has_size) {
  ...
  auto matcher = make_unique_del(
    sfhash_create_matcher(txt, txt + std::strlen(txt), nullptr),
    sfhash_destroy_matcher
  );

  SCOPE_ASSERT(!sfhash_matcher_has_size(matcher.get(), 122));
  SCOPE_ASSERT(sfhash_matcher_has_size(matcher.get(), 123));
  SCOPE_ASSERT(!sfhash_matcher_has_size(matcher.get(), 124));
}
...
SCOPE_TEST(has_hash) {
  ...
  SCOPE_ASSERT(!sfhash_matcher_has_hash(matcher.get(), 122, hashes[0].data()));
  SCOPE_ASSERT(!sfhash_matcher_has_hash(matcher.get(), 123, hashes[0].data()));
  SCOPE_ASSERT(sfhash_matcher_has_hash(matcher.get(), 123, hashes[1].data()));
  SCOPE_ASSERT(!sfhash_matcher_has_hash(matcher.get(), 124, hashes[0].data()));
}
